...
class NebulaPortal {
  ...
  init() {
    ...
    // Set up postprocessing (bloom effect)
...
const NebulaPortalComponent = () => {
  const containerRef = useRef(null);

  useEffect(() => {
    let portalInstance;
    if (containerRef.current) {
      portalInstance = new NebulaPortal(containerRef.current);
    }
    return () => {
      // Cleanup: remove the resize event listener and dispose the renderer.
      window.removeEventListener('resize', portalInstance.onResize);
      if (portalInstance && portalInstance.renderer) {
        portalInstance.renderer.dispose();
      }
    };
  }, []);

  return <div id="scene-container" ref={containerRef}></div>;
};

export default NebulaPortalComponent;
